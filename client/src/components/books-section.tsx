...
export default function BooksSection() {
  ...
  return (
    <section id="livros" className="py-24 relative overflow-hidden bg-secondary-light text-secondary-dark">
      {/* Background Pattern */}
      ...
      <div ref={sectionRef} className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-16">
          ...
          <h2 ref={titleRef} className="font-serif text-4xl md:text-5xl font-bold text-secondary-dark mb-6 opacity-0">
            Livros que Transformam Vidas
          </h2>
          <p ref={descRef} className="text-lg text-secondary-dark/80 max-w-2xl mx-auto opacity-0">
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-8 opacity-0">
          {books.map((book) => (
            <Card key={book.id} className="group overflow-hidden bg-secondary-dark rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 card-hover">
              <CardContent className="p-6 flex flex-col h-full">
                ...
                <h3 className="font-serif text-xl font-bold text-white mb-2">{book.title}</h3>
                <p className="text-white/80 text-sm mb-4 flex-1">{book.description}</p>
                <div className="flex items-center mb-4">
                  <Star className="w-4 h-4 text-gold mr-1" />
                  <span className="text-gold font-semibold mr-2">{book.rating}</span>
                  <span className="text-white/70 text-xs">({book.reviews} avaliações)</span>
                </div>
